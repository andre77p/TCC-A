...
export default function Produto() {
    ...
    async function salvar() {
        try {
            ...
            if (!id) {
                const v = await salvarProduto(nome, prevoProduto, destaque, idDepartamento, catSelecionadas);
                await salvarImagens(v.id, imagem1, imagem2, imagem3, imagem4);
    ...
    async function carregarDepartamentos() {
        const v = await listarDepartamentos();
        setDepartamentos(v);
    }


    async function carregarCategorias() {
        const v = await listarCategorias();
        setCategorias(v);
    }
    ...
    async function carregarProduto() {
        ...
        setIdProduto(v.info.id);
        setNome(v.info.produto);
        setPreco(v.info.preco.toString());
        setDestaque(v.info.destaque);
        setIdDepartamento(v.info.departamento);
        setCatSelecionadas(v.categorias);

        if (v.imagens.length > 0) {
            setImagem1(v.imagens[0]);
        }

        if (v.imagens.length > 1) {
            setImagem2(v.imagens[1]);
        }

        if (v.imagens.length > 2) {
            setImagem3(v.imagens[2]);
        }

        if (v.imagens.length > 3) {
            setImagem4(v.imagens[3]);
        }
    }
